...
public class WeaponTypeController {
	...
	public ResponseEntity<WeaponType> getTypeOfWeaponById(@PathVariable(value = ControllerAPI.PATH_VARIABLE_TYPE_ID) Long weaponId) throws BadRequestException {
	...
	public ResponseEntity<WeaponType> postTypeOfWeapon(@RequestBody @Valid WeaponTypeBean bean) {
		return new ResponseEntity<>(weaponService.postType(bean), HttpStatus.OK);
	}
	...
	public ResponseEntity<WeaponType> postTypeOfWeapon(@PathVariable(value = ControllerAPI.PATH_VARIABLE_TYPE_ID) long weaponTypeId, @RequestBody @Valid WeaponTypeBean bean) throws BadRequestException {
		return new ResponseEntity<>(weaponService.updateType(weaponTypeId, bean), HttpStatus.OK);
	}
	...
	public ResponseEntity<SuccessfulMessage> deleteWeaponType(@PathVariable(value = ControllerAPI.PATH_VARIABLE_TYPE_ID) long typeId) throws BadRequestException {
		weaponService.deleteType(typeId);
		return new ResponseEntity<>(new SuccessfulMessage("Weapon type %s has been successfully deleted", typeId), HttpStatus.OK);
	}
