...
//				log.info("Annotation is %s", JacksonUtils.getFullJson(constraint.getAnnotation()));
				...
				if (annotation instanceof javax.validation.constraints.Size) {
					...
					validationBeans.add(new ValidationBean().setName("Pattern").setFieldName(propertyName).setMessage(((javax.validation.constraints.Pattern) annotation).message()).setMessage(((javax.validation.constraints.Pattern) annotation).regexp()));
				}
