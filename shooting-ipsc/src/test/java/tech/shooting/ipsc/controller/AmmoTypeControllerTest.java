...
class AmmoTypeControllerTest {
	...
	void checkGetAllTypes() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.get(ControllerAPI.AMMO_TYPE_CONTROLLER + ControllerAPI.VERSION_1_0 + ControllerAPI.AMMO_TYPE_CONTROLLER_GET_ALL).header(Token.TOKEN_HEADER, userToken))
				.andExpect(MockMvcResultMatchers.status().isForbidden());

		// try access with judge role
		mockMvc.perform(MockMvcRequestBuilders.get(ControllerAPI.AMMO_TYPE_CONTROLLER + ControllerAPI.VERSION_1_0 + ControllerAPI.AMMO_TYPE_CONTROLLER_GET_ALL).header(Token.TOKEN_HEADER, judgeToken))
				.andExpect(MockMvcResultMatchers.status().isOk());

		// try access with admin role
		String contentAsString = mockMvc.perform(MockMvcRequestBuilders.get(ControllerAPI.AMMO_TYPE_CONTROLLER + ControllerAPI.VERSION_1_0 + ControllerAPI.AMMO_TYPE_CONTROLLER_GET_ALL).header(Token.TOKEN_HEADER, adminToken))
	...
	private void createFewVehicleType() {
		...
		types.add(new AmmoType().setName("AK-74"));
		types.add(new AmmoType().setName("AKC-74"));
	...
	void checkGetTypeById() throws Exception {
		...
		String contentAsString = mockMvc
				.perform(MockMvcRequestBuilders.get(ControllerAPI.AMMO_TYPE_CONTROLLER + ControllerAPI.VERSION_1_0 + ControllerAPI.AMMO_TYPE_CONTROLLER_GET_BY_ID.replace(ControllerAPI.REQUEST_TYPE_ID, type.getId().toString()))
						.header(Token.TOKEN_HEADER, adminToken))
				.andExpect(MockMvcResultMatchers.status().isOk()).andReturn().getResponse().getContentAsString();
		AmmoType listFromJson = JacksonUtils.fromJson(AmmoType.class, contentAsString);
	...
	void checkDeleteType() throws Exception {
		...
		// try access with unauthorized user role
		mockMvc.perform(
				MockMvcRequestBuilders.delete(ControllerAPI.AMMO_TYPE_CONTROLLER + ControllerAPI.VERSION_1_0 + ControllerAPI.AMMO_TYPE_CONTROLLER_DELETE_TYPE_BY_ID.replace(ControllerAPI.REQUEST_TYPE_ID, bean.getId().toString())))
				.andExpect(MockMvcResultMatchers.status().isUnauthorized());
