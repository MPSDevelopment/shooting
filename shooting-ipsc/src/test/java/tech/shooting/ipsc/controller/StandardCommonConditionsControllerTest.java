...
@ContextConfiguration(classes = { ValidationErrorHandler.class, IpscSettings.class, IpscMongoConfig.class, SecurityConfig.class, UserDao.class, DatabaseCreator.class, StandardCommonConditionsController.class,
		StandardCommonConditionsService.class })
class StandardCommonConditionsControllerTest {
	...
	void checkPostCommonCondition() throws Exception {
		StandardCommonConditionsBean bean = new StandardCommonConditionsBean().setCoefficient(20.0).setConditionsKz("fdsfdsfsfsd").setConditionsRus("fdsdfsfds").setUnits(units);
	...
	void checkGetAllCommonConditions() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.get(ControllerAPI.STANDARD_COMMON_CONDITION_CONTROLLER + ControllerAPI.VERSION_1_0 + ControllerAPI.STANDARD_COMMON_CONDITION_CONTROLLER_GET_ALL).header(Token.TOKEN_HEADER, userToken))
				.andExpect(MockMvcResultMatchers.status().isOk());
		// try access with judge role
		mockMvc.perform(MockMvcRequestBuilders.get(ControllerAPI.STANDARD_COMMON_CONDITION_CONTROLLER + ControllerAPI.VERSION_1_0 + ControllerAPI.STANDARD_COMMON_CONDITION_CONTROLLER_GET_ALL).header(Token.TOKEN_HEADER, judgeToken))
				.andExpect(MockMvcResultMatchers.status().isForbidden());
		// try access with admin role
		String contentAsString = mockMvc
				.perform(MockMvcRequestBuilders.get(ControllerAPI.STANDARD_COMMON_CONDITION_CONTROLLER + ControllerAPI.VERSION_1_0 + ControllerAPI.STANDARD_COMMON_CONDITION_CONTROLLER_GET_ALL).header(Token.TOKEN_HEADER, adminToken))
	...
	void checkGetCommonConditionById() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.get(
				ControllerAPI.STANDARD_COMMON_CONDITION_CONTROLLER + ControllerAPI.VERSION_1_0 + ControllerAPI.STANDARD_COMMON_CONDITION_CONTROLLER_GET_BY_ID.replace(ControllerAPI.REQUEST_COMMON_CONDITION_ID, condition.getId().toString())))
				.andExpect(MockMvcResultMatchers.status().isUnauthorized());
		...
		// try access with admin role
		String contentAsString = mockMvc
				.perform(MockMvcRequestBuilders.get(ControllerAPI.STANDARD_COMMON_CONDITION_CONTROLLER + ControllerAPI.VERSION_1_0
						+ ControllerAPI.STANDARD_COMMON_CONDITION_CONTROLLER_GET_BY_ID.replace(ControllerAPI.REQUEST_COMMON_CONDITION_ID, condition.getId().toString())).header(Token.TOKEN_HEADER, adminToken))
				.andExpect(MockMvcResultMatchers.status().isOk()).andReturn().getResponse().getContentAsString();
	...
	void checkDeleteCommonConditionById() throws Exception {
		...
		// try access with unauthorized user
		mockMvc.perform(MockMvcRequestBuilders.delete(ControllerAPI.STANDARD_COMMON_CONDITION_CONTROLLER + ControllerAPI.VERSION_1_0
				+ ControllerAPI.STANDARD_COMMON_CONDITION_CONTROLLER_DELETE_BY_ID.replace(ControllerAPI.REQUEST_COMMON_CONDITION_ID, condition.getId().toString()))).andExpect(MockMvcResultMatchers.status().isUnauthorized());

		// try access with user role
		mockMvc.perform(MockMvcRequestBuilders.delete(ControllerAPI.STANDARD_COMMON_CONDITION_CONTROLLER + ControllerAPI.VERSION_1_0
				+ ControllerAPI.STANDARD_COMMON_CONDITION_CONTROLLER_DELETE_BY_ID.replace(ControllerAPI.REQUEST_COMMON_CONDITION_ID, condition.getId().toString())).header(Token.TOKEN_HEADER, userToken))
				.andExpect(MockMvcResultMatchers.status().isOk());

		// try access with judge role
		mockMvc.perform(MockMvcRequestBuilders.delete(ControllerAPI.STANDARD_COMMON_CONDITION_CONTROLLER + ControllerAPI.VERSION_1_0
				+ ControllerAPI.STANDARD_COMMON_CONDITION_CONTROLLER_DELETE_BY_ID.replace(ControllerAPI.REQUEST_COMMON_CONDITION_ID, condition.getId().toString())).header(Token.TOKEN_HEADER, judgeToken))
				.andExpect(MockMvcResultMatchers.status().isForbidden());
		...
		// try access with admin role
		mockMvc.perform(MockMvcRequestBuilders.delete(ControllerAPI.STANDARD_COMMON_CONDITION_CONTROLLER + ControllerAPI.VERSION_1_0
				+ ControllerAPI.STANDARD_COMMON_CONDITION_CONTROLLER_DELETE_BY_ID.replace(ControllerAPI.REQUEST_COMMON_CONDITION_ID, condition.getId().toString())).header(Token.TOKEN_HEADER, adminToken))
				.andExpect(MockMvcResultMatchers.status().isOk());
	...
	void checkPutCommonCondition() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.put(ControllerAPI.STANDARD_COMMON_CONDITION_CONTROLLER + ControllerAPI.VERSION_1_0
				+ ControllerAPI.STANDARD_COMMON_CONDITION_CONTROLLER_PUT_CONDITION.replace(ControllerAPI.REQUEST_COMMON_CONDITION_ID, condition.getId().toString())).contentType(MediaType.APPLICATION_JSON_UTF8).content(json))
				.andExpect(MockMvcResultMatchers.status().isUnauthorized());

		mockMvc.perform(MockMvcRequestBuilders
				.put(ControllerAPI.STANDARD_COMMON_CONDITION_CONTROLLER + ControllerAPI.VERSION_1_0
						+ ControllerAPI.STANDARD_COMMON_CONDITION_CONTROLLER_PUT_CONDITION.replace(ControllerAPI.REQUEST_COMMON_CONDITION_ID, condition.getId().toString()))
		...
		assertEquals(bean.getUnits(), conditions.getUnits());
